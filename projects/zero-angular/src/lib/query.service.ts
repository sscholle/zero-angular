...
import { DEFAULT_TTL, Format, HumanReadable, Query, ResultType, Schema, TTL, TypedView } from "@rocicorp/zero";
...
export class QueryService {
    private viewStore = new ViewStore();

    constructor(private zero: ZeroService<Schema>) {
    }
    useQuery<
    ...
    >(
    ...
    ): Observable<QueryResult<TReturn>> {
        let enabled = true;
        ...
        const z = this.zero.getZero();
        const view = this.viewStore.getView(
            z.clientID,
            query as Query<TSchema, TTable, TReturn>,
            enabled,
            ttl,
        );

        const subject = new BehaviorSubject(view.getSnapshot());
        const unsubscribe = view.subscribeReactInternals(() => {
            subject.next(view.getSnapshot());
        });

        return new Observable<QueryResult<TReturn>>((observer) => {
  ...
  #onData = (
    ...
    const data =
      snap === undefined
        ...
        : (JSON.parse(JSON.stringify(snap)) as HumanReadable<TReturn>);
